...
- **What ports need to be open for communication with the SAGE3 server, what encryption and protocols are used and what authentication?**
  - The HTTPS port (443) is required. All the communications are initiated over HTTPS using TLS 1.2: HTTPS calls, secure WebSocket communication and WebRTC. If the server has screen sharing enabled, two more ports must reach the server for the video itself: UDP 7882 (WebRTC media) and TCP 7881 (fallback for networks that block UDP). Screen sharing is served by the SAGE3 server's own video component (LiveKit); no third party is involved. Servers still on the legacy Twilio backend send screen-share media through Twilio, over HTTPS.
