...
chart_info = {
    ...
    "Line Chart": {
        ...
        "example": "Bacteria growth over time: time (categorical), bacteria count (numerical)",
    },
    # "Column Histogram": {
...
def chart_info_filter(attributes):
    ...
    scoped_charts_instructions = "\n".join(
        [chart_decision_tree[key] for key in scoped_charts.keys()]
    )
...
def chart_info_filter_type_scoped(attributes, csv_context_scoped):
    ...
    csv_context_scoped = [
        column for column in csv_context_scoped if column["Column Name"] in attributes
    ]
    csv_categoricals = [
        attribute
        for attribute in csv_context_scoped
        if attribute["Data Type"] == "Categorical"
    ]
    csv_numericals = [
        attribute
        for attribute in csv_context_scoped
    ...
    scoped_charts = {
        ...
        if len(csv_categoricals)
        == value.get("attribute_counts", {}).get("categorical", -1)
        and len(csv_numericals)
        == value.get("attribute_counts", {}).get("numerical", -1)
    }
    # scoped_charts_instructions = "\n".join([chart_decision_tree[key] for key in scoped_charts.keys()])
    scoped_charts_instructions = json.dumps(
        {
            key: {
                "description": chart_info[key]["description"],
                "example": chart_info[key]["example"],
            }
            for key in scoped_charts.keys()
        },
        indent=2,
    )
