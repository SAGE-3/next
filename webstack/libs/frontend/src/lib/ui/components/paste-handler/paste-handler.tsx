          ...
          Array.from(event.clipboardData.files).forEach(async (file) => {
            if (file.type.startsWith('image/')) {
              ...
              createApp({
                ...
                boardId: props.boardId,
                position: { x: xDrop, y: yDrop, z: 0 },
                size: { width: 700, height: 400, depth: 0 },
                rotation: { x: 0, y: 0, z: 0 },
                type: 'Stickie',
                state: { text: textcontent, fontSize: 32, color: user.data.color || 'yellow' },
                raised: true,
                dragging: false,
              });
            } else {
      ...
      if (pastedText) {
        ...
        if (isValid) {
          ...
          createApp({
            ...
            rotation: { x: 0, y: 0, z: 0 },
            type: 'Stickie',
            state: { text: pastedText, fontSize: 32, color: user.data.color || 'yellow' },
            raised: true,
            dragging: false,
          });
        }
