...
function ToolbarComponent(props: App): JSX.Element {
  ...
  //     update(props._id, { description: 'SageCell> kernel ' + name });
  //   }
  // }, [s.kernel, kernels]);
  ...
  // // renders the selection options for the kernel dropdown menu
  const RenderSelect = (kernels: string) => {
    if (kernels) {
      try {
        const parsed = JSON.parse(kernels);
        if (Object.keys(parsed).length > 0) {
          return Object.keys(parsed).map((kernel) => (
  ...
  // if (s.kernels && kernels.length === 0) {
  //   try {
  //     const parsed = JSON.parse(s.kernels);
  //     if (Object.keys(parsed).length > 0) {
  //       setKernels(
  //         Object.keys(parsed).map((kernel) => ({
  //           id: kernel,
  //           name: parsed[kernel],
  //         }))
  //       );
      ...
      <HStack>
        ...
        <ButtonGroup isAttached size="xs" colorScheme="teal">
          <Tooltip placement="top-start" hasArrow={true} label={'Decrease Font Size'} openDelay={400}>
            <Button
              isDisabled={s.fontSize < 0.5}
              onClick={() => updateState(props._id, { fontSize: Math.max(0.5, s.fontSize / 1.2) })}
          ...
          <Tooltip placement="top-start" hasArrow={true} label={'Increase Font Size'} openDelay={400}>
            <Button
              isDisabled={s.fontSize >= 2}
              onClick={() => updateState(props._id, { fontSize: Math.min(s.fontSize * 1.2, 2) })}
              _hover={{ opacity: 0.7, transform: 'scaleY(1.3)' }}
            >
              <MdAdd />
            </Button>
