  ...
  return (
    <div>
      <HStack spacing="5" display="flex" justify="center">
        ...
        <div
          style={{
            display: s.currentPage - 1 !== 1 && s.currentPage !== 1 ? 'block' : 'none',
          }}
        >
          <Menu>
            ...
            <Portal>
              <MenuList display="flex" maxW="sm" zIndex="dropdown" overflowY="scroll">
                {s.pageNumbers.map((page: number) => (
                  <Button key={page} onClick={(e) => paginater(page)} variant={s.currentPage === page ? 'solid' : 'link'} size="sm">
                    {page}
                  </Button>
                ))}
              </MenuList>
        ...
        {pageNumbers.map((page: number) => (
          <Button key={page} onClick={(e) => paginater(page)} variant={s.currentPage === page ? 'solid' : 'link'}>
            {page}
          </Button>
        ))}
        <div
          style={{
            display: s.currentPage + 1 !== s.pageNumbers.length && s.currentPage !== s.pageNumbers.length ? 'block' : 'none',
          }}
        >
          <Menu size="xs">
            <MenuButton as={IconButton} icon={<FiMoreHorizontal />} variant="link" size="xs">
          ...
          <Portal>
            <MenuList>
              {rowsPerPageArr.map((rows: number, key: number) => {
                return (
                  <MenuItem key={key} onClick={() => handleRowDisplayCount(rows)}>
                    {rows}
                  </MenuItem>
                );
              })}
      ...
      <Box fontSize="xs" font-style="oblique" float="right">
        <p>
          Showing {s.indexOfFirstRow} - {s.indexOfLastRow - 1} of {s.totalRows} rows
        </p>
        <p>
          Page {s.currentPage} of {s.pageNumbers[s.pageNumbers.length - 1]}
        </p>
      </Box>
    </div>
  );
...
function AppComponent(props: App): JSX.Element {
  ...
  function handleLoadData() {
    ...
    setRunning(true);
    updateState(props._id, { executeInfo: { executeFunc: 'load_data', params: { url: s.dataUrl } } });
    console.log('new value of executeInfo is ');
    console.log(s.executeInfo);
    console.log('----');
    console.log(s);
    ...
    cols.forEach((cell: any) => {
      if (!s.selectedCols?.includes(info)) {
        ...
        updateState(props._id, {
          messages: info.charAt(0).toUpperCase() + info.slice(1) + ' column selected ---' + ' Selected Columns: ' + checked.toString(),
        });
      } else {
        const unchecked = (() => s.selectedCols?.filter((item: string) => item != info))();
        updateState(props._id, { selectedCols: unchecked });
        updateState(props._id, {
          messages: info.charAt(0).toUpperCase() + info.slice(1) + ' column unselected ---' + ' Selected Columns: ' + unchecked.toString(),
        });
      }
    });
  }
  ...
  function tableSort() {
    ...
    updateState(props._id, { executeInfo: { executeFunc: 'table_sort', params: { selected_cols: s.selectedCols } } });
    console.log(s.executeInfo);
    console.log('----');
    console.log(s);
  }
  ...
  function dropColumns() {
    ...
    updateState(props._id, { executeInfo: { executeFunc: 'drop_columns', params: { selected_cols: s.selectedCols } } });
    console.log(s.executeInfo);
    console.log('----');
    console.log(s);
    //
  ...
  function dropRows() {
    ...
    updateState(props._id, { executeInfo: { executeFunc: 'drop_rows', params: { selected_rows: s.selectedRows } } });
    console.log(s.executeInfo);
    console.log('----');
    console.log(s);
    //
  ...
  function transposeTable() {
    ...
    updateState(props._id, { executeInfo: { executeFunc: 'transpose_table', params: {} } });
    console.log(s.executeInfo);
    console.log('----');
    console.log(s);
  }
  ...
  function restoreTable() {
    ...
    updateState(props._id, { executeInfo: { executeFunc: 'restore_table', params: {} } });
    console.log(s.executeInfo);
    console.log('----');
    console.log(s);
  }
  ...
  function columnSort(column: any) {
    ...
    updateState(props._id, { executeInfo: { executeFunc: 'column_sort', params: { selected_col: s.selectedCol } } });
    console.log(s.executeInfo);
    console.log('----');
    console.log(s);
  }
  ...
  function dropColumn(column: any) {
    updateState(props._id, { executeInfo: { executeFunc: 'drop_column', params: { selected_col: s.selectedCol } } });
  ...
  return (
    <AppWindow app={props}>
      <>
          ...
          <div className="searchContainer">
            ...
            {headers?.map((col: string, index: number) => (
              <div className="card">
                ...
                <input type="text" id={col} className="search" onKeyDown={(e: React.FormEvent<HTMLInputElement>) => enterSearch(e, col)} />
              </div>
            ))}
          </div>

          <HStack position="absolute" top="35px" right="15px">
            <Box>
              <Popover isOpen={isOpen} onOpen={onOpen} onClose={onClose} placement="right" closeOnBlur={false}>
                <PopoverTrigger>
                  <Button rightIcon={<TbWorldDownload />}>New Dataset</Button>
                </PopoverTrigger>
                <PopoverContent p={5}>
                  <FocusLock returnFocus persistentFocus={true}>
                    ...
                    <Stack spacing={4}>
                      <FormControl>
                        ...
                        <Input size="md" type="text" value={s.dataUrl} onChange={handleUrlChange} />
        ...
        <div style={{ display: s.totalRows !== 0 ? 'none' : 'block' }} className="URL-Container">
          <InputGroup size="md">
            <Input type="text" value={s.dataUrl} onChange={handleUrlChange} placeholder={'URL here'} />
            ...
            <Table variant="simple" className="originalChakra" width="auto" whiteSpace="nowrap" maxHeight="22rem">
              ...
              <Tbody>
                {data?.map((row: any, rowIndex: number) => (
                  <Tr key={indices[rowIndex]}>
                    <Td key={indices[rowIndex]} className="indexTd" onClick={(e) => handleRowClick(indices[rowIndex])}>
                      {indices[rowIndex]}
                    </Td>
        ...
        <div className="Pagination-Container" style={{ display: s.pageNumbers.length !== 0 ? 'block' : 'none' }}>
          <Pagination {...props} />
        </div>

        <div className="Message-Container">
          <VStack spacing={3}>
            <Box fontWeight="bold">Message Center</Box>
