...
export function LinksArrows(props: { links: Link[] }) {
  ...
  const appsById = useMemo(() => {
    ...
    for (const app of apps) {
      // Preserve the first match, as Array.find did.
      if (!byId.has(app._id)) byId.set(app._id, app);
    }
    return byId;
  }, [apps]);
    ...
    <div className="arrows-container" style={{ pointerEvents: 'none', touchAction: 'auto' }}>
      ...
      >
        ...
        {links.map((link, linkIndex) => {
          ...
          const sourceApp = appsById.get(sourceAppId);
          ...
          const similarLinks = links.filter((otherLink, otherIndex) => {
            ...
            const otherSourceApp = appsById.get(otherSource);
            const otherTargetApp = appsById.get(otherTarget);

            if (!otherSourceApp || !otherTargetApp) return false;

            // Check for exact same connection or reverse connection
            const exactMatch =
              (sourceAppId === otherSource && targetAppId === otherTarget) || (sourceAppId === otherTarget && targetAppId === otherSource);

            if (exactMatch) return true;

            // Check for nearby apps that would create visually overlapping arrows
            const distance = (a1: any, a2: any) =>
              Math.sqrt(Math.pow(a1.data.position.x - a2.data.position.x, 2) + Math.pow(a1.data.position.y - a2.data.position.y, 2));
            ...
            return (sourceDistance < threshold && targetDistance < threshold) || (crossDistance1 < threshold && crossDistance2 < threshold);
          });
