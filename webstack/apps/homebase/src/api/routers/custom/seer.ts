...
/**
 * Route forwarding the seer calls to the seer server
 */
export function SeerAPIRouter() {
  console.log('SeerAPI> router for SeerAPI', config.seer.url);

  const router = createProxyMiddleware({
    target: config.seer.url,
    changeOrigin: true,
    pathRewrite: { '^/api/seer': '' },
