...
/**
 * Get and format the latest data from Sage Node
 * @param query 
 * @returns array of date, and metric data in the form of {x: string, y: number}
 */
export async function getSageNodeData(query: SageNodeQuery) {
  try {
    ...
    const formattedData = filteredParsedData.map((d) => {
      //console.log("sage node date", d.timestamp);
      return {
        ...
        y: query.filter.name === 'env.pressure' ? d.value / 100 : d.value, // Convert pressure from Pa to millibars
      };
    });
...
/**
 * Get and format the latest data from Mesonet
 * @param query
 * @returns array of date, and metric data in the form of {x: string, y: number}
 */
export async function getMesonetData(query: MesonetQuery) {
  try {
...
/**
 * Merge data from Sage Node and Mesonet into one dataset
 * @param data 
 * @returns 
 */
export async function mergeData(data: RAPIDQueries) {
  try {
