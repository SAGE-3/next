 ...
 */
export function VmsRouter() {
  console.log('Vms> router for co sage container manager (vnc backend)', config.vms.managerUrl);

  const router = createProxyMiddleware({
    target: config.vms.managerUrl,
    changeOrigin: true,
    pathRewrite: { '^/api/vm': '/api/vm' },
