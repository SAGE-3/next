...
import { AuthHTTPService, CreateUserModal, useUserStore, EditUserModal, CreateRoomModal, useRoomStore, useBoardStore, CreateBoardModal, useAppStore } from "@sage3/frontend";
...
export function HomePage() {
  ...
  return (
    <div>
      ...
      {(currentRoom) ? <CreateBoardModal roomId={currentRoom.id} isOpen={newBoardModal} onClose={() => setNewBoardModal(false)}></CreateBoardModal> : null}
          ...
          {rooms.map(room => {
            ...
            return (
              <Tag
                ...
                onClick={() => { setCurrentRoom(room); subByRoomId(room.id) }}
              >
                <TagLabel >{room.name} - {room.id}</TagLabel>
          ...
          {(currentRoom) ? boards.map(board => {
            return (
              <Tag
                ...
                borderRadius='full'
                variant='solid'
                colorScheme={board.color}
              >
                <TagLabel>{board.name} - {board.roomId}</TagLabel>
