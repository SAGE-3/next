...
def uploadFile(roomId, boardId, filename, fullname, posx, posy, w=400, h=400):
    ...
    payload = {
        # specify taarget position will open the file, otherwise not
        # 'targetX': posx,
        # 'targetY': posy,
        # 'targetWidth': w,
        # 'targetHeight': h,
    ...
    # POST
    r = requests.post(web_server + '/api/assets/upload',
...
async def main():
    ...
    print('room0', room0)
    print('board0', board0)
    uploadFile(room0['_id'], board0['_id'],  "test.jpg", "test.jpg", 200, 200, 400, 400)
