...
def createStickie(boardId, text, x=300, y=300, width=600, height=320, color="#FF7EB9"):
    ...
    payload = {
        ...
        'optionalData': {'value': {'text': text, 'color': color}},
    }
    r = boardAction(boardId, payload)
    print('createStickie>', r.text)


def fillBoard(bid, bw, bh):
    """Fill a board with stickies
    """
    colors = ['#FC8181', '#F6AD55', '#F6E05E',
              '#68D391', '#4FD1C5', '#63b3ed', '#B794F4']
    sw = 720
    sh = 410
    with open('moon.txt') as f:
        speech = f.read()
        words = re.split('\s+', speech)
        block = 8*9
        per = math.ceil(len(words) / block)
        blocks = []
        for i in range(0, len(words), per):
            ...
            blocks.append(s)
    k = 0
    for x in range(75, bw-sw, sw+150):
        for y in range(100, bh-sh, sh+150):
            createStickie(bid, blocks[k], x, y, sw,
                          sh, colors[random.randint(0, 6)])
            k = k + 1
...
@sio.event
def connect():
    global myboard_id, bwidth, bheight
    ...
    fillBoard(myboard_id, bwidth, bheight)
